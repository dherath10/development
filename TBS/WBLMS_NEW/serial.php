 ...
 */
    ...
    class serialPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $field = new StringField('ISSN');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
            $field = new StringField('Pages');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('serialssearch', $this->dataset,
                array('ISSN', 'Pages', 'Item_ID_Title'),
                array($this->RenderText('ISSN'), $this->RenderText('Pages'), $this->RenderText('Item ID')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('serialasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('ISSN', $this->RenderText('ISSN')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('Pages', $this->RenderText('Pages')));
        ...
        {
            //
            // View column for ISSN field
            //
            $column = new TextViewColumn('ISSN', 'ISSN', $this->dataset);
            $column->SetOrderable(true);
            
            /* <inline edit column> */
            //
            // Edit column for ISSN field
            //
            $editor = new TextEdit('issn_edit');
            $editor->SetSize(50);
            $editor->SetMaxLength(50);
            $editColumn = new CustomEditColumn('ISSN', 'ISSN', $editor, $this->dataset);
            ...
            //
            // View column for Pages field
            //
            $column = new TextViewColumn('Pages', 'Pages', $this->dataset);
            $column->SetOrderable(true);
            
            /* <inline edit column> */
            //
            // Edit column for Pages field
            //
            $editor = new TextEdit('pages_edit');
            $editor->SetSize(50);
            $editor->SetMaxLength(50);
            $editColumn = new CustomEditColumn('Pages', 'Pages', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $column->SetEditOperationColumn($editColumn);
            /* </inline edit column> */
        ...
        public function GetModalGridDeleteHandler() { return 'serial_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'serialGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new serialPage("serial.php", "serial", GetCurrentUserGrantForDataSource("serial"), 'UTF-8');
        $Page->SetShortCaption('Serial');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Serial');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("serial"));
