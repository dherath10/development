 ...
 */
    ...
    class reservationPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
                ...
                '`reservation`');
            $field = new IntegerField('Reserve_ID', null, null, true);
            ...
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
            $field = new DateField('Res_Date');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
            $field = new StringField('Res_Time');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
            $field = new DateField('Notification_Date');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
            $field = new StringField('Fulfilled');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
            $this->dataset->AddLookupField('User_ID', 'user', new IntegerField('User_ID', null, null, true), new StringField('First_Name', 'User_ID_First_Name', 'User_ID_First_Name_user'), 'User_ID_First_Name_user');
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('reservationssearch', $this->dataset,
                array('Reserve_ID', 'User_ID_First_Name', 'Res_Date', 'Res_Time', 'Notification_Date', 'Fulfilled', 'Item_ID_Title'),
                array($this->RenderText('Reserve ID'), $this->RenderText('User ID'), $this->RenderText('Res Date'), $this->RenderText('Res Time'), $this->RenderText('Notification Date'), $this->RenderText('Fulfilled'), $this->RenderText('Item ID')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('reservationasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('Reserve_ID', $this->RenderText('Reserve ID')));
            ...
            $field->SetIsNotNull(true);
            $lookupDataset->AddField($field, false);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateLookupSearchInput('User_ID', $this->RenderText('User ID'), $lookupDataset, 'User_ID', 'First_Name', false));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateDateTimeSearchInput('Res_Date', $this->RenderText('Res Date')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('Res_Time', $this->RenderText('Res Time')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateDateTimeSearchInput('Notification_Date', $this->RenderText('Notification Date')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('Fulfilled', $this->RenderText('Fulfilled')));
        ...
        {
            //
            // View column for Reserve_ID field
            //
            $column = new TextViewColumn('Reserve_ID', 'Reserve ID', $this->dataset);
            ...
            //
            $column = new TextViewColumn('User_ID_First_Name', 'User ID', $this->dataset);
            $column->SetOrderable(true);
            ...
            $editColumn = new LookUpEditColumn(
                ...
                'User_ID', 
                $editor, 
                $this->dataset, 'User_ID', 'First_Name', $lookupDataset);
            ...
            //
            // View column for Res_Date field
            //
            $column = new DateTimeViewColumn('Res_Date', 'Res Date', $this->dataset);
            $column->SetDateTimeFormat('Y-m-d');
            $column->SetOrderable(true);
            
            /* <inline edit column> */
            //
            // Edit column for Res_Date field
            //
            $editor = new DateTimeEdit('res_date_edit', false, 'Y-m-d', GetFirstDayOfWeek());
            $editColumn = new CustomEditColumn('Res Date', 'Res_Date', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $column->SetEditOperationColumn($editColumn);
            ...
            //
            // View column for Res_Time field
            //
            $column = new DateTimeViewColumn('Res_Time', 'Res Time', $this->dataset);
            $column->SetDateTimeFormat('H:i:s');
            $column->SetOrderable(true);
            
            /* <inline edit column> */
            //
            // Edit column for Res_Time field
            //
            $editor = new TimeEdit('res_time_edit');
            $editColumn = new CustomEditColumn('Res Time', 'Res_Time', $editor, $this->dataset);
            ...
            $editColumn = new CustomEditColumn('Res Time', 'Res_Time', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $editColumn->SetInsertDefaultValue($this->RenderText('%CURRENT_TIME%'));
            $this->ApplyCommonColumnEditProperties($editColumn);
            $column->SetInsertOperationColumn($editColumn);
            /* </inline insert column> */
            $column->SetDescription($this->RenderText(''));
            ...
            //
            // View column for Notification_Date field
            //
            $column = new DateTimeViewColumn('Notification_Date', 'Notification Date', $this->dataset);
            $column->SetDateTimeFormat('Y-m-d');
            $column->SetOrderable(true);
            
            /* <inline edit column> */
            //
            // Edit column for Notification_Date field
            //
            $editor = new DateTimeEdit('notification_date_edit', false, 'Y-m-d', GetFirstDayOfWeek());
            $editColumn = new CustomEditColumn('Notification Date', 'Notification_Date', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $column->SetEditOperationColumn($editColumn);
            /* </inline edit column> */
            ...
            //
            // View column for Fulfilled field
            //
            $column = new TextViewColumn('Fulfilled', 'Fulfilled', $this->dataset);
            $column->SetOrderable(true);
            
            /* <inline edit column> */
            //
            // Edit column for Fulfilled field
            //
            $editor = new TextEdit('fulfilled_edit');
            $editor->SetSize(50);
            $editor->SetMaxLength(50);
            $editColumn = new CustomEditColumn('Fulfilled', 'Fulfilled', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $column->SetEditOperationColumn($editColumn);
        ...
        public function GetModalGridDeleteHandler() { return 'reservation_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'reservationGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new reservationPage("reservation.php", "reservation", GetCurrentUserGrantForDataSource("reservation"), 'UTF-8');
        $Page->SetShortCaption('Reservation');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Reservation');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("reservation"));
